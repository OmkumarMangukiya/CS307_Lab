...
class Node:
    ...
    def __hash__(self):
        return hash("".join(self.state.flatten().astype(str)))

    def __str__(self):
        return str(self.state)

    def __eq__(self, other):
        return hash("".join(self.state.flatten().astype(str))) == hash(
            "".join(other.state.flatten().astype(str))
        )
...
steps, solution_depth = agent.run()
print(f"Solution found in {solution_depth} steps!")
print("\nSolution path:")
agent.print_nodes()

# Performance analysis
print("\n" + "="*60)
print("MEMORY AND TIME REQUIREMENTS FOR SOLVING PUZZLE-8")
print("="*60)

depths = [5, 10, 20, 50, 100, 150, 200, 250, 300]
goal_state = np.array([[1, 2, 3], [4, 5, 6], [7, 8, 0]])
times_taken = {}
mems = {}

print("Depth (d)\tTime (seconds)\tMemory (bytes)")
print("-"*50)

for depth in depths:
    time_taken = 0
    mem = 0
    iterations = 5  # Reduced iterations for higher depths
    successful_runs = 0
    ...
    for i in range(iterations):
        try:
            ...
            if agent.goal_node is not None:  # Solution found
    ...
    if successful_runs > 0:
        time_taken /= successful_runs
        mem = mem / successful_runs
        times_taken[depth] = time_taken
        mems[depth] = mem
        print(f"{depth}\t\t{time_taken:.3f}\t\t{int(mem)}")
    else:
        print(f"{depth}\t\tTimeout\t\tN/A")